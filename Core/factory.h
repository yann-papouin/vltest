...
//! \file Factory.h Interface for the Factory class.
...
//////////////////////////////////////////////////////////////////////
//! \class Factory
/*! \brief Factory : Factory for all geometrical objects
 *  this class is a singleton
 */
//////////////////////////////////////////////////////////////////////
class GVCORE_EXPORT Factory /*: public vl::Object*/
{
public:
...
public:
	...
	//GLC_3DRep createPointCloud(const std::vector<GLC_Point3d>& pointList, const QColor& color);

	//GLC_3DRep createPointCloud(const std::vector<GLC_Point3df>& pointList, const QColor& color);
	...
	//GLC_3DViewInstance createRectangle(const GLC_Point3d& point, const GLC_vl::dvec3& normal, double l1, double l2);

	////! Create the representation of a cutting from the given 3d point, normal, lenght and material
	//GLC_3DViewInstance createCuttingPlane(const GLC_Point3d& point, const GLC_vl::dvec3& normal, double l1, double l2, GLC_Material* pMat);

	////! Create a GLC_World from a QFile
	//GLC_World createWorldFromFile(QFile &file, std::stringList* pAttachedFileName= NULL) const;
	...
	Rep3D createRep3DFromFile(const std::string&) const;
	...
	//GLC_Material* createMaterial(const std::string &textureFullFileName) const;

	////! create a material textured with a QImage
	//GLC_Material* createMaterial(const QImage &) const;

	////! Create a GLC_Texture
	//GLC_Texture* createTexture(const std::string &textureFullFileName) const;

	////! Create a GLC_Texture with a QImage
	//GLC_Texture* createTexture(const QImage &, const std::string& imageFileName= std::string()) const;
	...
	//static std::vector<GLC_WorldReaderPlugin*> worldReaderPlugins();

	//! Return true if the given file extension can be loaded
	static bool canBeLoaded(const std::string& extension);

	//! Return an handle to the plugin tu use for the given file
	//static GLC_WorldReaderHandler* loadingHandler(const std::string& fileName);
...
private:
	...
	static Factory* m_pFactory;

	//! The list off worldReader plugins
// 	static std::vector<GLC_WorldReaderPlugin*> m_WorldReaderPluginList;

	//! The supported extension set
	static std::set<std::string> m_SupportedExtensionSet;
